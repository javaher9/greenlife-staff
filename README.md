...
## اتصال Greenlife API Server (v42)

پس از اجرای migration، مدیر سیستم از مسیر `/settings/api/` این دو مقدار را ثبت می‌کند:

- `Base URL`: شامل پروتکل، IP، پورت و مسیر پایه؛ مقدار پیش‌فرض `http://192.168.40.33:81/gl-api`
- `X-API-Key`: فقط در فرم مدیر وارد و به‌صورت رمزگذاری‌شده در دیتابیس ذخیره می‌شود.

از همین صفحه می‌توان اتصال `POST /crm/finance_month` و ارسال آزمایشی
`POST /sms/send` را جداگانه بررسی کرد. همگام‌سازی مالی، ردیف‌های ماه جاری شمسی را
با کلید پایدار `formValueId + group` به‌صورت Upsert وارد می‌کند. ارسال تأیید نوبت
به‌صورت پیش‌فرض خاموش است و پس از تست موفق توسط مدیر فعال می‌شود.
